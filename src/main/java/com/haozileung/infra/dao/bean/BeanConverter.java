...
public class BeanConverter {
	 ...
	 *            the ignore list
	 */
	private static void copySameProperties(Object target, Object source,
		...
		for (PropertyDescriptor targetPd : targetPds) {
			...
			try {
				...
				if (sourcePd != null && sourcePd.getReadMethod() != null) {
					...
					// 自定义转换
					value = typeConvert(sourcePd.getPropertyType(),
							targetPd.getPropertyType(), value);
	 ...
	 * @return the object
	 */
	private static Object typeConvert(Class<?> sourcePropertyType,
			Class<?> targetPropertyType, Object value) {
		...
		for (Map.Entry<Class<?>, TypeConverter> entry : converters.entrySet()) {
			...
			return converter.convert(sourcePropertyType, targetPropertyType,
					value);
		}
		return value;
	}
	...
	private static PropertyDescriptor[] getPropertyDescriptors(
		...
		IntrospectionCache introspectionCache = IntrospectionCache
				.forClass(beanClass);
