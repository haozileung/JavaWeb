...
public class SqlAssembleUtils {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(SqlAssembleUtils.class);
	 ...
	 * @return bound sql
	 */
	public static BoundSql buildInsertSql(Object entity, Criteria criteria,
			NameHandler nameHandler) {

		Class<?> entityClass = getEntityClass(entity, criteria);
		List<AutoField> autoFields = (criteria != null ? criteria
				.getAutoFields() : new ArrayList<AutoField>());

		List<AutoField> entityAutoField = getEntityAutoField(entity,
				AutoField.UPDATE_FIELD);
	 ...
	 * @return bound sql
	 */
	public static BoundSql buildUpdateSql(Object entity, Criteria criteria,
			NameHandler nameHandler) {
		...
		while (iterator.hasNext()) {
			...
			// 白名单 黑名单
			if (criteria != null
					&& !CollectionUtils.isEmpty(criteria.getIncludeFields())
					&& !criteria.getIncludeFields().contains(
							autoField.getName())) {
				continue;
			} else if (criteria != null
					&& !CollectionUtils.isEmpty(criteria.getExcludeFields())
					&& criteria.getExcludeFields()
							.contains(autoField.getName())) {
				continue;
			}
	 ...
	 */
	private static List<AutoField> getEntityAutoField(Object entity,
			int operateType) {
	 ...
	 * @return
	 */
	public static BoundSql buildDeleteSql(Class<?> clazz, Long id,
			NameHandler nameHandler) {
	 ...
	 * @return bound sql
	 */
	public static BoundSql buildDeleteSql(Object entity, Criteria criteria,
			NameHandler nameHandler) {
	 ...
	 * @return bound sql
	 */
	public static BoundSql buildByIdSql(Class<?> clazz, Long id,
			Criteria criteria, NameHandler nameHandler) {

		Class<?> entityClass = (clazz == null ? criteria.getEntityClass()
				: clazz);
		String tableName = nameHandler.getTableName(entityClass);
		String primaryName = nameHandler.getPKName(entityClass);
	 ...
	 * @return bound sql
	 */
	public static BoundSql buildQuerySql(Object entity, Criteria criteria,
			NameHandler nameHandler) {
	 ...
	 * @return bound sql
	 */
	public static BoundSql buildListSql(Object entity, Criteria criteria,
			NameHandler nameHandler) {

		BoundSql boundSql = SqlAssembleUtils.buildQuerySql(entity, criteria,
				nameHandler);
	 ...
	 * @return bound sql
	 */
	public static BoundSql buildCountSql(Object entity, Criteria criteria,
			NameHandler nameHandler) {
	...
	public static String buildColumnSql(Class<?> clazz,
			NameHandler nameHandler, List<String> includeField,
		...
		for (PropertyDescriptor pd : pds) {
			...
			if (!CollectionUtils.isEmpty(includeField)
					&& !includeField.contains(fieldName)) {
				continue;
			} else if (!CollectionUtils.isEmpty(excludeField)
					&& excludeField.contains(fieldName)) {
				continue;
			}
